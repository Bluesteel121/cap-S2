...
<body class="bg-gray-50">
    <header class="bg-gradient-to-r from-[#115D5B] to-[#0d4a47] text-white py-4 px-6 shadow-lg">
                ...
                <nav class="flex space-x-8 px-6">
                    <a href="?status=all" 
                       class="tab-button flex items-center py-4 px-2 text-sm font-medium <?php echo $status_filter === 'all' ? 'active' : 'text-gray-500 hover:text-gray-700'; ?>">
                        All Papers (<?php echo array_sum($status_counts); ?>)
                    </a>
                    ...
                    <a href="?status=rejected" 
                       class="tab-button flex items-center py-4 px-2 text-sm font-medium <?php echo $status_filter === 'rejected' ? 'active' : 'text-gray-500 hover:text-gray-700'; ?>">
                    ...
                    <div class="paper-card bg-white rounded-lg shadow-md p-6">
                        ...
                        <div class="mb-4">
                            <p class="text-sm text-gray-700 line-clamp-3">
                                <strong>Abstract:</strong> <?php echo htmlspecialchars(substr($paper['abstract'], 0, 300)) . '...'; ?>
                            </p>
            ...
            <div class="bg-gradient-to-r from-[#115D5B] to-[#0d4a47] text-white p-4 rounded-t-xl flex justify-between items-center">
                ...
                <div class="flex gap-2">
                    <button onclick="openFullscreenPDFFromModal()" 
                            class="bg-white bg-opacity-20 hover:bg-opacity-30 px-3 py-1 rounded transition">
        ...
        <div class="bg-white rounded-xl shadow-2xl max-w-2xl w-full mx-4">
            ...
            <form id="reviewForm" class="p-6">
                ...
                <div class="mb-6">
                    ...
                    <textarea id="reviewerComments" name="reviewer_comments" rows="8" required
                ...
                <div class="flex space-x-3">
                    <button type="submit" 
                            class="bg-[#115D5B] hover:bg-[#0d4a47] text-white px-6 py-3 rounded-lg transition">
    ...
    <div id="fullscreenPDF" class="pdf-viewer-fullscreen hidden">
        <div class="absolute top-4 right-4 z-10 flex gap-2">
            <div class="bg-gray-800 px-3 py-2 rounded-lg text-white text-sm">
    ...
    <script>
            ...
            document.getElementById('detailsContent').innerHTML = `
                <div class="space-y-6">
                    <div>
                        ...
                        <div class="text-gray-700 bg-gray-50 p-4 rounded-lg max-h-60 overflow-y-auto">${escapeHtml(paper.abstract)}</div>
                    </div>

                    ${paper.methodology ? `
                    <div>
                        <h4 class="font-semibold text-gray-800 mb-2">Methodology:</h4>
                        <div class="text-gray-700 bg-gray-50 p-4 rounded-lg">${escapeHtml(paper.methodology)}</div>
                    </div>
                    ` : ''}
                    ...
                    <div>
                        ...
                        <div class="text-gray-700 bg-yellow-50 p-4 rounded-lg border-l-4 border-yellow-400">${escapeHtml(paper.reviewer_comments)}</div>
                        ${paper.reviewed_by ? `<p class="text-sm text-gray-500 mt-2">Reviewed by: ${escapeHtml(paper.reviewed_by)} on ${new Date(paper.review_date).toLocaleDateString()}</p>` : ''}
                    </div>
                    ` : ''}
        ...
        function viewPaperPDF(paperId) {
            ...
            document.getElementById('pdfStatus').innerHTML = '<i class="fas fa-spinner fa-spin text-4xl text-gray-400 mb-4"></i><p class="text-gray-500">Loading PDF...</p>';
